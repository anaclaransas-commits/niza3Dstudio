...
import { X, Clock, Package, Eye, Heart } from 'lucide-react';
...
/* ─── Product card ───────────────────────────────────── */
/* ─── ProductCard Melhorado (com clique no card) ───────────────────────────────────── */
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        {waLink ? (
          <a 
            ...
            target="_blank" 
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}   // ← Importante: impede duplo clique
...
function ProductModal({ 
  ...
  const waLink = `https://wa.me/${whatsapp}?text=Olá! Gostei do produto *${encodeURIComponent(product.name)}* e quero um orçamento.`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[95vh] overflow-hidden shadow-2xl">
        ...
        <div className="flex flex-col lg:flex-row">
          <div className="lg:w-1/2 bg-slate-50 p-8">
            <img src={product.imageUrl} alt={product.name} className="w-full rounded-2xl shadow-xl" />
          </div>
          ...
          <div className="lg:w-1/2 p-8 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              ...
              {product.defaultWeightG && (
                <div>
                  <p className="text-xs uppercase text-slate-400">Peso</p>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-auto block w-full py-4 rounded-2xl font-bold text-center text-white text-lg hover:scale-105 transition-all"
              style={{ backgroundColor: accent }}
            >
...
export function CatalogPublic() {
  ...
  {filtered.map(p => (
    <ProductCard 
      ...
      whatsapp={whatsapp || ''} 
      ctaLabel={ctaLabel}
      onQuickView={openProduct}     // ← Adicionado aqui
    />
  ))}
</div>
        ) : (
